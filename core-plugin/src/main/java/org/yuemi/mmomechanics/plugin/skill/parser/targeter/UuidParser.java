...
import org.yuemi.mmomechanics.plugin.skill.targeter.entity.single.UuidTargeter;
    ...
    public @Nullable Targeter parse(@Nullable String name) {
        ...
        if (clean.contains("{") && clean.contains("}")) {
            ...
            for (String pair : options.split(",")) {
                ...
                if (parts.length == 2 && (parts[0].trim().equalsIgnoreCase("uuid") || parts[0].trim().equalsIgnoreCase("id"))) {
                    uuidExpr = parts[1].trim();
                }
            }
        }
        return new UuidTargeter(uuidExpr);
    }
}
